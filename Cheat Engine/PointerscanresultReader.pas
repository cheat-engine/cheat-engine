...
uses windows, sysutils, classes, cefuncproc, newkernelhandler, symbolhandler, math;
...
type TPointerscanresultReader=class
  private
    ...
    cacheSize: integer;
    cache: pointer;
    function InitializeCache(i: uint64): boolean;
  public
    procedure resyncModulelist;
    procedure saveModulelistToResults(s: Tstream);
    function getModulename(modulenr: integer): string;
    function getModuleBase(modulenr: integer): dword;
    function getPointer(i: uint64): PPointerscanResult; overload;
    function getPointer(i: uint64; var pointsto: dword): PPointerscanResult; overload;
...
function TPointerscanresultReader.InitializeCache(i: uint64): boolean;
var j: integer;
    actualread: dword;
begin
  result:=false;
...
function TPointerscanresultReader.getModuleBase(modulenr: integer): dword;
{pre: modulenr must be valid, so not -1 }
begin
  result:=dword(modulelist.Objects[modulenr]);
end;
...
function TPointerscanresultReader.getPointer(i: uint64; var pointsto: dword): PPointerscanResult;
{
