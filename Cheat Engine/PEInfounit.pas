...
begin
  ...
  ImageNTHeader:=PImageNtHeaders(dword(memorycopy)+PImageDosHeader(memorycopy)^._lfanew);
  if ImageNTHeader^.Signature<>IMAGE_NT_SIGNATURE then
    exit; //not supported // if OptionalHeader.Magic<>
  ...
  begin
    ...
    if i=0 then
    begin
      //exports, not really used for loading, but let's check
...
//     do me
  ...
  showmessage('Loaded at '+inttohex(dword(loadedmodule),8) + ' entrypoint='+inttohex(dword(loadedmodule)+ImageNTHeader^.OptionalHeader.AddressOfEntryPoint,8));
   
  //PImageDosHeader(memorycopy)^.
end;
...
procedure TfrmPEInfo.Button2Click(Sender: TObject);
begin
  onshow(self);
end;
