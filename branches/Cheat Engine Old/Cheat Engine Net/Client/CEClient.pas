...
uses Unit2, processwindow, Unit1, Unit3, changetimerunit, formsettingsunit,
  MemoryBrowserFormUnit, formScanningUnit, FoundCodeUnit,disassembler;
...
begin
  case message.wParam of
    ...
    begin
      ...
      mainform.values[data.dw1]:=data.st;
      freemem(data.st);
      freemem(data);
    end;
    ...
    begin
      st:=pointer(message.LParam);
      mainform.FoundList.Items.Add(st);
      freemem(st);
    end;
...
begin
  ...
  try
    ...
    if Reg.OpenKey('\Software\Cheat Engine\Network',true) then
    begin
  ...
  case command of
    ...
    SC_ValueUpdated:
       begin
         ...
         receivebuf(output[0],3);
         receivebuf(output[3],output[2]);
         output[output[2]+3]:=0;

         getmem(data,sizeof(tdata));
         data.dw1:=pword(@output[0])^;
         st:=pchar(@output[3]);
         getmem(data.st,length(st)+1);
         copymemory(data.st,@st[1],length(st));
         data.st[length(st)]:=#0;
