...
function GetWindow_hook(hwnd:HWND; uCmd:UINT):HWND; stdcall;
...
begin
  ...
  GetWindowThreadProcessId(hwnd,@winprocess);
  if ((hwnd=scansettings.hyperscanwindow) or ((scansettings.CEProcessID<>0) and (winprocess=scansettings.CEProcessID))) then
    result:=0
  else
